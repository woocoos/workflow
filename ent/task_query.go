...
type TaskQuery struct {
	...
	withTaskIdentities      *IdentityLinkQuery
	modifiers               []func(*sql.Selector)
	loadTotal               []func(context.Context, []*Task) error
	withNamedTaskIdentities map[string]*IdentityLinkQuery
...
func (tq *TaskQuery) QueryProcInst() *ProcInstQuery {
	query := (&ProcInstClient{config: tq.config}).Query()
...
func (tq *TaskQuery) QueryTaskIdentities() *IdentityLinkQuery {
	query := (&IdentityLinkClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withTaskIdentities: tq.withTaskIdentities.Clone(),
...
// WithProcInst tells the query-builder to eager-load the nodes that are connected to
// the "proc_inst" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithProcInst(opts ...func(*ProcInstQuery)) *TaskQuery {
	query := (&ProcInstClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withProcInst = query
	return tq
}

// WithTaskIdentities tells the query-builder to eager-load the nodes that are connected to
// the "task_identities" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithTaskIdentities(opts ...func(*IdentityLinkQuery)) *TaskQuery {
	query := (&IdentityLinkClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withTaskIdentities = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withProcInst; query != nil {
		if err := tq.loadProcInst(ctx, query, nodes, nil,
			func(n *Task, e *ProcInst) { n.Edges.ProcInst = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withTaskIdentities; query != nil {
		if err := tq.loadTaskIdentities(ctx, query, nodes,
			func(n *Task) { n.Edges.TaskIdentities = []*IdentityLink{} },
			func(n *Task, e *IdentityLink) { n.Edges.TaskIdentities = append(n.Edges.TaskIdentities, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range tq.withNamedTaskIdentities {
		if err := tq.loadTaskIdentities(ctx, query, nodes,
			func(n *Task) { n.appendNamedTaskIdentities(name) },
			func(n *Task, e *IdentityLink) { n.appendNamedTaskIdentities(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (tq *TaskQuery) loadProcInst(ctx context.Context, query *ProcInstQuery, nodes []*Task, init func(*Task), assign func(*Task, *ProcInst)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		fk := nodes[i].ProcInstID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "proc_inst_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) loadTaskIdentities(ctx context.Context, query *IdentityLinkQuery, nodes []*Task, init func(*Task), assign func(*Task, *IdentityLink)) error {
...
// WithNamedTaskIdentities tells the query-builder to eager-load the nodes that are connected to the "task_identities"
...
func (tq *TaskQuery) WithNamedTaskIdentities(name string, opts ...func(*IdentityLinkQuery)) *TaskQuery {
	...
	if tq.withNamedTaskIdentities == nil {
		tq.withNamedTaskIdentities = make(map[string]*IdentityLinkQuery)
	}
	tq.withNamedTaskIdentities[name] = query
	return tq
}
